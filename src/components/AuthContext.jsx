...
export function AuthProvider({ children }) {
  ...
  const [modalView, setModalView] = useState('login'); // 'login' | 'register' | 'forgot' | 'profile'
  ...
  const login = async (email, password) => {
    ...
    if (res.accessToken) {
      localStorage.setItem('as_customer_token', res.accessToken);
      const profileRes = await fetchCustomerDetails(res.accessToken);
      if (profileRes.success && profileRes.customer) {
        ...
        const loggedInUser = {
          ...
          email: c.email,
          phone: c.phone || '',
          avatar: `https://api.dicebear.com/7.x/adventurer/svg?seed=${c.email}`
        };
    ...
    localStorage.setItem('as_user', JSON.stringify(fallbackUser));
    setLoginModalOpen(false);
    return { success: true, user: fallbackUser };
  };

  const register = async (customerData) => {
    const res = await customerRegister(customerData);
    if (!res.success) return res;
    ...
    if (res.accessToken) {
      ...
      if (profileRes.success && profileRes.customer) {
        const c = profileRes.customer;
        const loggedInUser = {
          firstName: c.firstName || customerData.firstName || '',
          lastName: c.lastName || customerData.lastName || '',
          name: `${c.firstName || customerData.firstName || ''} ${c.lastName || customerData.lastName || ''}`.trim() || c.email,
          email: c.email,
    ...
    const fallbackUser = {
      firstName: customerData.firstName || customerData.email.split('@')[0],
      lastName: customerData.lastName || '',
      name: `${customerData.firstName || ''} ${customerData.lastName || ''}`.trim() || customerData.email,
      email: customerData.email,
      phone: customerData.phone || '',
      avatar: `https://api.dicebear.com/7.x/adventurer/svg?seed=${customerData.email}`
    };
    setUser(fallbackUser);
    localStorage.setItem('as_user', JSON.stringify(fallbackUser));
    setLoginModalOpen(false);
            ...
            <body>
              <div class="card">
                <svg class="logo" viewBox="0 0 74 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <path d="M29.1 12.7C29.1 16.2 26.5 18.6 23.2 18.6C19.9 18.6 17.3 16.2 17.3 12.7C17.3 9.2 19.9 6.8 23.2 6.8C26.5 6.8 29.1 9.2 29.1 12.7ZM26.4 12.7C26.4 10.4 24.8 8.9 23.2 8.9C21.6 8.9 20 10.4 20 12.7C20 15 21.6 16.5 23.2 16.5C24.8 16.5 39.5 15 26.4 12.7Z" fill="#EA4335"/>
              ...
              <script>
                function selectUser(name, email, avatar) {
                  window.opener.postMessage({
                    ...
                    user: { name, email, avatar, firstName: name.split(' ')[0] }
  ...
  const logout = () => {
    const token = typeof window !== 'undefined' ? localStorage.getItem('as_customer_token') : null;
    if (token) customerLogout(token);
    setUser(null);
    if (typeof window !== 'undefined') {
      localStorage.removeItem('as_user');
      localStorage.removeItem('as_customer_token');
    }
  };
